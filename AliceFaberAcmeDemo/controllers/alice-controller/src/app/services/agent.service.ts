...
export class AgentService {
  ...
  createInvitation(): Observable<any> {
    return this.http.post<any>("/connections/create-invitation", {}).pipe(
      switchMap((response: any) => of(response)),
      catchError(this.handleError<any>("createInvitation", null))
    );
  }

  acceptInvitation(connectionId: string): Observable<any> {
    return this.http
      .post<any>(`/connections/${connectionId}/accept-invitation`, {
        conn_id: connectionId,
      })
      .pipe(
        switchMap((response: any) => of(response)),
        catchError(this.handleError<any>("acceptInvitation", null))
      );
  }
