...
router.post(
  ...
  [
    ...
    check("invitation_object").custom((value) => {
      try {
        JSON.parse(value);
        return true;
      } catch (error) {
  ...
  handleAcceptConnectionPost,
  handleAcceptConnectionGet
);
...
async function handleAcceptConnectionPost(req, res, next) {
  ...
  const response = await agentService.receiveInvitation(
    req.body.invitation_object
  );
  console.log(response);
  console.log(response.connection_id);
  await agentService.acceptInvitation(response.connection_id);
...
router.get("/:id/remove", async function (req, res, next) {
  ...
  const redirectUrl = `/connections/${
    state === "invitation" ? "pending" : "active"
  }`;
